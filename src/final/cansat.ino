...
Point dest={35.676958,139.475775};
...
double calc_included_angle(Point o,Point a,Point b){
  //直線OAから直線OBに行くために、時計回りにどれだけ回る必要があるのか求める。
  Point va={a.x-o.x,a.y-o.y};
  Point vb={b.x-o.x,b.y-o.y};
  double theta1=atan2(va.x,va.y);
  double theta2=atan2(vb.x,vb.y);
...
//リングバッファで過去10回分の位置情報を保管。
Point prev_location[10];
int location_id=0;
  ...
  while (1) {
    while (hs.available() > 0) {
      ...
      if (gps.location.isUpdated()) {
        ...
        prev_location[location_id].x = gps.location.lat();
        prev_location[location_id].y = gps.location.lng();
        location_id++;
        location_id%=10;
      }
    }
...
Point calc_accurate_location(){
  Point ret={0,0};
  for(int i=0;i<10;i++){
    ret.x+=prev_location[i].x;
    ret.y+=prev_location[i].y;
  }
  ret.x/=10;
  ret.y/=10;
  return ret;
...
/** 目標地点へ走行 */
void drive() {
  Point s1=calc_accurate_location();
  forward(255);
  delay(20000);
  stop();
  Point s2=calc_accurate_location();
  double theta = calc_included_angle(s1,s2,dest)*180/acos(-1);  //度数法
  rotate(theta);
  delay(2000);
}
void drive_for_rotate_test(){
  forward(255);
  delay(1000);
  stop();
  rotate(90);
...
void turn_round(){
  forward_balanced(255,192);
  delay(20000);
  stop();
  forward_balanced(192,255);
  delay(20000);
  stop();
  delay(2000);
}
...
void forward_balanced(int lpwm,int rpwm) {
  if (lpwm < 0) lpwm = 0;
  ...
  digitalWrite(pin_motor_A[0], LOW);
  digitalWrite(pin_motor_A[1], HIGH);
  ledcWrite(CHANNEL_A, lpwm);

  // 右モータ（CW，時計回り）
  digitalWrite(pin_motor_B[1], LOW);
  digitalWrite(pin_motor_B[0], HIGH);
  ledcWrite(CHANNEL_B, rpwm);
}

void rotate(double theta){
  double target=sensorVal.yaw-theta;
  if(target<-180){
    target+=360;
  }else if(target>180){
    target-=360;
  ...
  if(theta<0){
    rotate_left();
    while(true){
      if(abs(sensorVal.yaw-target)<10){
        break;
      }
      Serial.printf("%f %f\n",sensorVal.yaw,target);
    }
  }else{
    rotate_right();
    while(true){
